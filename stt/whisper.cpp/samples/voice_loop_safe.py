"""
AI Pet Voice Loop with PiCar-X Integration and Safety Features
Main application for Raspberry Pi 5 with PiCar-X robot platform, local LLM, and collision detection
"""
import subprocess
import time
import os
...
from safety import SafetyMonitor, SafePiCarXController
...
def get_llm_response(user_text, config, safety_status=None):
    """Get response from local LLM"""
    safety_context = ""
    if safety_status:
        if not safety_status.get("safe"):
            safety_context = "\nNote: There is an obstacle nearby, so you cannot move forward right now."
        elif safety_status.get("emergency_stop"):
            safety_context = "\nNote: Emergency stop is active. Wait for clearance."
...
acknowledge the command in your response with enthusiasm.{safety_context}
...
def execute_movement_command(text, controller):
    ...
    if action == "forward":
        success = controller.forward(duration=command.get("duration", 2.0))
        return success
    elif action == "backward":
        success = controller.backward(duration=command.get("duration", 2.0))
        return success
    elif action == "turn_left":
        success = controller.turn_left(duration=command.get("duration", 1.5))
        return success
    elif action == "turn_right":
        success = controller.turn_right(duration=command.get("duration", 1.5))
        return success
    elif action == "stop":
        controller.stop()
        return True
...
def main():
    ...
    if config.get("enable_picarx"):
        base_controller = PiCarXController(
        ...
        safe_controller = SafePiCarXController(base_controller, safety_monitor)
        
        # Start safety monitoring
        if config.get("enable_safety_monitoring", True):
            safety_monitor.start_monitoring()
        
        picarx_controller = safe_controller
    ...
    if config.get('enable_picarx'):
        ...
        print(f"   Safety: {'Enabled' if config.get('enable_safety_monitoring', True) else 'Disabled'}")
    print()
    ...
    try:
        while True:
            ...
            # Get safety status
            safety_status = None
            if picarx_controller and hasattr(picarx_controller, 'get_safety_status'):
                safety_status = picarx_controller.get_safety_status()
            
            # Get LLM response
            response = get_llm_response(user_text, config, safety_status)
